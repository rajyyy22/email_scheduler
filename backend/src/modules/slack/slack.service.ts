...
import { ErrorCode, type SlackConnectionDto } from '../../types/index.js';

export class SlackService {
  private static slackClient = new WebClient();

  public static async generateSlackConnectUrl(userId: string): Promise<string> {
    const state = crypto.randomBytes(32).toString('hex');
    await redis.set(
      `oauth:v1:slack:${state}`,
  ...
  public static async handleSlackCallback(code: string, state: string): Promise<void> {
    ...
    const { userId } = JSON.parse(rawState) as { userId: string };
    ...
    if (!response.ok || !response.incoming_webhook) {
      throw new AppError(
        ...
        `Slack OAuth failed: ${response.error || 'No incoming webhook received'}`,
      );
    }

    const team = response.team as { id: string; name: string };
    const webhook = response.incoming_webhook as {
      channel: string;
      channel_id: string;
  ...
  public static async getSlackStatus(userId: string): Promise<SlackConnectionDto> {
